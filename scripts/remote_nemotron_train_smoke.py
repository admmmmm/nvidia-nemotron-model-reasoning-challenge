...
def build_python_script(
    ...
    if target_modules:
        lines.extend(
            [
                ...
                "module_samples = [n for n, m in model.named_modules() if 'linear' in n or 'proj' in n][:20]",
                "print('module_samples=', module_samples, flush=True)",
                "if hasattr(model, 'enable_input_require_grads'):",
                "    model.enable_input_require_grads()",
                "    print('input_grad_mode=enable_input_require_grads', flush=True)",
                "else:",
                "    def _make_inputs_require_grad_after_lora(module, inputs, output):",
                "        output.requires_grad_(True)",
                "    try:",
                "        model.get_input_embeddings().register_forward_hook(_make_inputs_require_grad_after_lora)",
                "        print('input_grad_mode=embedding_hook', flush=True)",
                "    except Exception as exc:",
                "        print('input_grad_mode=failed', repr(exc), flush=True)",
            ]
        )
    else:
    ...
    lines.extend(
        [
            ...
            "trainable_param_names = [name for name, p in model.named_parameters() if p.requires_grad][:20]",
            "print('trainable_param_names=', trainable_param_names, flush=True)",
            ...
            "print('loss_requires_grad=', bool(loss.requires_grad), flush=True)",
            "print('loss_grad_fn=', type(loss.grad_fn).__name__ if loss.grad_fn is not None else None, flush=True)",
            "if hasattr(outputs, 'logits'):",
            "    print('logits_requires_grad=', bool(outputs.logits.requires_grad), flush=True)",
        ]
    )
